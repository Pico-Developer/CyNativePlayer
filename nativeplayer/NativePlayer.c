...
#include "src/ijkplayer/ijkplayer/ijkplayer.h"
#include "src/inc/utils/ncfence/ncfence.h"
...
int CypressPlayer_getState(CypressPlayer *mp) {
    if (!mp || !mp->mp) return CYPRESS_PLAYER_STATE_END;
...
void CypressPlayer_setSpatialAudioMode(CypressPlayer* player, int spatialAudioMode) {
    CypressPlayer_setOptionLong(player, FFP_OPT_CATEGORY_PLAYER,
                            "spatial_audio_mode", spatialAudioMode);
}

void CypressPlayer_enableSpatialAudio(CypressPlayer* player, bool enable) {
    ijkmp_set_spatial_audio_enable(player->mp, enable);
}

void CypressPlayer_setSpatialAudioEntity(CypressPlayer* player, int64_t entity_id) {
    ijkmp_set_spatial_audio_entity(player->mp, entity_id);
}

void CypressPlayer_setSpatialAudioContainer(CypressPlayer* player, int64_t audio_bind_id) {
    ijkmp_set_spatial_audio_container(player->mp, audio_bind_id);
}

void CypressPlayer_setSeekMode(CypressPlayer* player, CypressPlayerSeekMode seek_mode) {
    FFSeekMode ff_seek_mode = FF_SEEK_MODE_PREVIOUS;
    switch (seek_mode) {
        case CYPRESS_PLAYER_SEEK_MODE_PREVIOUS:
            ff_seek_mode = FF_SEEK_MODE_PREVIOUS;
            break;
        case CYPRESS_PLAYER_SEEK_MODE_EXACT:
            ff_seek_mode = FF_SEEK_MODE_ACCURATE;
            break;
        case CYPRESS_PLAYER_SEEK_MODE_AUTO:
            ff_seek_mode = FF_SEEK_MODE_AUTO;
            break;
    }

    ijkmp_set_seek_mode(player->mp, ff_seek_mode);
}
...
int CypressPlayer_setABPlaybackRange(CypressPlayer *player, int64_t fromPositionMs, int64_t toPositionMs) {
    return ijkmp_set_ab_playback_range(player->mp, fromPositionMs, toPositionMs);
}
