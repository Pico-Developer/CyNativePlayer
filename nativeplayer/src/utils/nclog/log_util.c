...
static void cy_log(Appender appender, TraceType type, LogLevel level, const char *tag, const char *fmt_str, va_list args) {
    ...
    char android_log_enable[PROP_VALUE_MAX] = {0};
    if (__system_property_get("cypressplayer.debug.log.enable", android_log_enable) > 0 && strcmp(android_log_enable, "1") == 0) {
        __android_log_print(android_log_level_map[level], tag, "%s", msg);
    }
