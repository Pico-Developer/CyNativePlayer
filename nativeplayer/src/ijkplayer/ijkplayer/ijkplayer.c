...
inline static void ijkmp_destroy(IjkMediaPlayer *mp)
{
    ...
    // msg_thread depend on ffplayer->msg_queue
...
static void ijkmp_destroy_p(IjkMediaPlayer **pmp)
{
    if (!pmp)
        return;
...
void ijkmp_release(IjkMediaPlayer **pmp) {
    if (!pmp || !*pmp)
        return;

    PerfMonitor *perf_monitor = (*pmp)->perf_monitor;
    if (perf_monitor) perf_monitor_on_release_start(perf_monitor);

    CYLOGI("msg_thread_id %ld tid %d", (*pmp)->msg_thread_tid, gettid());

    ijkmp_shutdown(*pmp);
    ijkmp_destroy_p(pmp);

    if (perf_monitor) {
        perf_monitor_on_release_end(perf_monitor);
        perf_monitor_report(perf_monitor);
        perf_monitor_destroy(&perf_monitor);
    }
}
...
{
    if (new_state == mp->mp_state) return;

    mp->mp_state = new_state;
    MPTRACE("ijkmp_change_state: %d", new_state);
    ffp_notify_msg2(mp->ffplayer, FFP_MSG_PLAYBACK_STATE_CHANGED, new_state);
}

static int32_t get_package_name(char *package_name_buf, int32_t buf_size) {
    ...
    int32_t read_size = read(fd, package_name_buf, buf_size - 1);
    if (read_size <= 0) {
        close(fd);
        return -1;
    }
    close(fd);
    package_name_buf[read_size] = '\0';
    return 0;
}
...
IjkMediaPlayer *ijkmp_create(int (*msg_loop)(void*))
{
    ...
    mp->start_when_prepared = 0;
    mp->msg_thread_tid = 0;

    PerfMonitor *perf_monitor = NULL;
    const char *package_name = "unknown";
    char buf[128] = {0};
    if (get_package_name(buf, sizeof(buf)) == 0) {
        package_name = buf;
    }
    if (perf_monitor_init(&perf_monitor, package_name, getpid()) != 0) {
        CYLOGE("failed to create performance monitor");
    }
    mp->perf_monitor = mp->ffplayer->perf_monitor = perf_monitor;
    return mp;
...
void ijkmp_shutdown_l(IjkMediaPlayer *mp)
{
    ...
    if (mp->ffplayer) {
        ...
        // Note: ffp_wait_stop_l() will call ffp_stop_l()
...
{
    CYLOGI("msg loop thread [%d]", gettid());
...
static int ijkmp_prepare_async_l(IjkMediaPlayer *mp)
{
    ...
    MPTRACE("ijkmp_prepare_async_l state: %d", mp->mp_state);
    MPST_RET_IF_EQ(mp->mp_state, MP_STATE_IDLE);
    // MPST_RET_IF_EQ(mp->mp_state, MP_STATE_INITIALIZED);
    MPST_RET_IF_EQ(mp->mp_state, MP_STATE_ASYNC_PREPARING);
    MPST_RET_IF_EQ(mp->mp_state, MP_STATE_PREPARED);
    ...
    if (retval != 0) {
        ffp_notify_msg2(mp->ffplayer, FFP_MSG_ERROR, retval);
        return retval;
    }
...
static int ijkmp_stop_l(IjkMediaPlayer *mp)
{
    ...
    MPTRACE("%s: ffp_wait_stop_l()", __func__);
    int retval = ffp_wait_stop_l(mp->ffplayer);
    MPTRACE("%s: ffp_wait_stop_l() ret %d", __func__, retval);
    return retval;
}
...
int ijkmp_stop(IjkMediaPlayer *mp)
{
    ...
    PerfMonitor *perf_monitor = mp->perf_monitor;
    if (perf_monitor) {
        perf_monitor_on_stop_start(perf_monitor);
    }
    ...
    if (retval != 0) {
        MPTRACE("ijkmp_stop() failed, ret: %d", retval);
        return retval;
    }

    MPTRACE("ijkmp_stop() wait msg_thread %p start", mp->msg_thread);
    if (mp->msg_thread) {
        SDL_WaitThread(mp->msg_thread, NULL);
        mp->msg_thread = NULL;
    }
    MPTRACE("ijkmp_stop() wait msg_thread %p end", mp->msg_thread);
    ...
    if (!ffp->codec_reuse_enable) {
        res_mgr_reset(ffp->res_mgr);
    }
    pthread_mutex_unlock(&mp->mutex);

    MPTRACE("ijkmp_stop()=%d\n", retval);

    // TODO: reset some fields in FFPlayer

    if (perf_monitor) {
        perf_monitor_on_stop_end(perf_monitor);
        // TODO: perf monitor couldn't distinguish between stop and release now
        perf_monitor_report(perf_monitor);
        perf_monitor_reset(perf_monitor);
    }
    return retval;
}
...
int ijkmp_seek_to_l(IjkMediaPlayer *mp, long msec)
{
    ...
    if (mp->mp_state == MP_STATE_IDLE || mp->mp_state == MP_STATE_INITIALIZED) {
        ffp_seek_to_l(mp->ffplayer, msec);
        return 0;
    }
...
static int ijkmp_get_state_l(IjkMediaPlayer *mp)
{
    return mp->mp_state;
}
...
int ijkmp_get_state(IjkMediaPlayer *mp)
{
    assert(mp);
    pthread_mutex_lock(&mp->mutex);
    int state = ijkmp_get_state_l(mp);
    pthread_mutex_unlock(&mp->mutex);
    return state;
}
...
static long ijkmp_get_current_position_l(IjkMediaPlayer *mp)
{
    if (mp->seek_req)
        return mp->seek_msec;
    if (mp->mp_state == MP_STATE_COMPLETED && !mp->has_seek_after_complete) {
        return ffp_get_ab_end_pos_l(mp->ffplayer);
    }
...
void ijkmp_set_spatial_audio_enable(IjkMediaPlayer *mp, bool enable)
{
    assert(mp);
    pthread_mutex_lock(&mp->mutex);
    ffp_set_spatial_audio_enable(mp->ffplayer, enable);
    pthread_mutex_unlock(&mp->mutex);
}

void ijkmp_set_spatial_audio_entity(IjkMediaPlayer *mp, int64_t entity_id)
{
    assert(mp);
    pthread_mutex_lock(&mp->mutex);
    ffp_set_spatial_audio_entity(mp->ffplayer, entity_id);
    pthread_mutex_unlock(&mp->mutex);
}

void ijkmp_set_spatial_audio_container(IjkMediaPlayer *mp, int64_t audio_bind_id)
{
    assert(mp);
    pthread_mutex_lock(&mp->mutex);
    ffp_set_spatial_audio_container(mp->ffplayer, audio_bind_id);
    pthread_mutex_unlock(&mp->mutex);
}

void ijkmp_set_seek_mode(IjkMediaPlayer *mp, FFSeekMode seek_mode)
{
    assert(mp);
    pthread_mutex_lock(&mp->mutex);
    ffp_set_seek_mode(mp->ffplayer, seek_mode);
    pthread_mutex_unlock(&mp->mutex);
}

int ijkmp_set_ab_playback_range(IjkMediaPlayer *mp, int64_t from_ms, int64_t to_ms)
{
    assert(mp);
    pthread_mutex_lock(&mp->mutex);
    int retval = ffp_set_ab_playback_range(mp->ffplayer, from_ms, to_ms);
    pthread_mutex_unlock(&mp->mutex);
    return retval;
}
...
int ijkmp_get_msg(IjkMediaPlayer *mp, AVMessage *msg, int block)
{
    ...
    while (1) {
        ...
        // NOTE: sdk uses FFP_REQ_STOP to quit instead of using msg_queue_abort(),
        // which will lost some msg that sdk needs
        if (msg->what == FFP_REQ_STOP) {
            // other thread may use this flag(by calling get/put), so sdk call it finally
        ...
        case FFP_MSG_PREPARED:
            ...
            int retval = 0;
            if (mp->ffplayer->is->start_on_prepared_for_single_play) {
                retval = ffp_start_l(mp->ffplayer);
            }

            if (mp->ffplayer->start_on_prepared || mp->ffplayer->is->start_on_prepared_for_single_play && retval == 0) {
        ...
        case FFP_MSG_SEEK_COMPLETE:
            ...
            mp->seek_req = 0;
            mp->seek_msec = 0;
            if (mp->mp_state == MP_STATE_COMPLETED) mp->has_seek_after_complete = 1;
            pthread_mutex_unlock(&mp->mutex);
            break;
        ...
        case FFP_REQ_START:
            ...
            pthread_mutex_lock(&mp->mutex);
            if (mp->mp_state == MP_STATE_ASYNC_PREPARING) {
                mp->ffplayer->is->start_on_prepared_for_single_play = 1;
                pthread_mutex_unlock(&mp->mutex);
                break;
            }
            ...
            if (0 == ikjmp_chkst_start_l(mp->mp_state)) {
                ...
                if (mp->restart) {
                    if (mp->restart_from_beginning) {
                        ...
                        int64_t start_pos = 0;
                        if (mp->ffplayer->start_time != AV_NOPTS_VALUE && mp->ffplayer->start_time > 0) {
                            start_pos = mp->ffplayer->start_time / 1000;
                        }
                        retval = ffp_start_from_l(mp->ffplayer, start_pos);
                        if (retval == 0)
                            ijkmp_change_state_l(mp, MP_STATE_STARTED);
                    } else {
        ...
        case FFP_REQ_PAUSE:
            ...
            pthread_mutex_lock(&mp->mutex);
            if (mp->mp_state == MP_STATE_ASYNC_PREPARING) {
                mp->ffplayer->is->start_on_prepared_for_single_play = 0;
                pthread_mutex_unlock(&mp->mutex);
                break;
            }
        ...
        case FFP_MSG_ERROR:
            ...
            pthread_mutex_unlock(&mp->mutex);
            if (mp->ffplayer->perf_monitor) perf_monitor_on_error(mp->ffplayer->perf_monitor, msg->arg1);
            break;
        }
