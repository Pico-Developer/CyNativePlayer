...
#include "ff_ffseek_mode.h"

// #include "perf_monitor/perf_monitor.h"
// because ff_ffplay_def is used in ijksdl/NativePlayer/...
// and we don't want to export it, please don't include
// perf_monitor here
...
#define FAST_BUFFERING_CHECK_PER_MILLISECONDS   (1)
...
#define VIDEO_PICTURE_QUEUE_SIZE_DEFAULT    (VIDEO_PICTURE_QUEUE_SIZE_MAX)
...
typedef struct Clock {
    ...
    double last_get_time; /* the time we get last time */
} Clock;
...
typedef struct FFSeekInfo {
    SDL_mutex *mutex;

    // couldn't be auto mode
    // sdk will decide the mode according to the video information and user_req_seek_mode
    ...
    double last_rendered_pts;
} FFSeekInfo;

typedef struct FFGopInfo {
    double gop_duration;
    double max_frame_gap;
} FFGopInfo;
...
typedef struct VideoState {
    ...
    int start_on_prepared_for_single_play;
...
typedef struct FFPlayer {
    ...
    long last_get_position;

    int         spatial_audio_mode;
    bool        enable_spatial_audio;
    int         enable_spatial_audio_changed;
    int64_t     entity_id;
    int         entity_id_changed;
    int64_t     audio_bind_id;
    int         audio_bind_id_changed;

    int codec_reuse_enable;
    ResMgr *res_mgr;

    FFSeekMode user_req_seek_mode;

    SDL_mutex *stereo_volume_mutex;
    float stereo_volume[2];
    bool stereo_volume_changed;
    bool user_req_set_stereo_volume;
...
inline static void ffp_reset_internal(FFPlayer *ffp)
{
    ...
    ffp->seek_at_start          = AV_NOPTS_VALUE;
    ...
    ffp->codec_reuse_enable              = 0; // option
    ffp->res_mgr                         = NULL;

    ffp->perf_monitor                   = NULL;

    ffp->user_req_seek_mode             = FF_SEEK_MODE_PREVIOUS;

    ffp->stereo_volume[0]               = 1.0f;
    ffp->stereo_volume[1]               = 1.0f;
