...
#if defined(__ANDROID__)
#define printf(...) CYLOGD(__VA_ARGS__)
#endif
...
// to notify that we have a new surface to set
static AVPacket set_surface_pkt;
...
static int packet_queue_put_private(PacketQueue *q, AVPacket *pkt)
{
    ...
    pkt1->pkt = *pkt;
    pkt1->next = NULL;
    if (pkt == &flush_pkt || pkt == &set_surface_pkt)
        q->serial++;
...
static int packet_queue_put(PacketQueue *q, AVPacket *pkt)
{
    ...
    if (pkt != &flush_pkt && pkt != &set_surface_pkt && ret < 0)
        av_packet_unref(pkt);
...
static int decoder_decode_frame(FFPlayer *ffp, Decoder *d, AVFrame *frame, AVSubtitle *sub) {
    ...
    for (;;) {
        ...
        } else if (ffp_is_set_surface_packet(&pkt)) {
            CYLOGI("%s: ffp_is_set_surface_packet, ignore %p", __func__, pkt.data);
            if (d->finished) {
...
static void frame_queue_get_snapshot(VideoState *is, FrameQueue *f,
                                     int *frame_count,
                                     double *duration_sum,
                                     ...
                                     int *last_serial)
{
    ...
    double first_pts = NAN;
    double total_duration = 0;
    double tail_pts = NAN;
    int serial = -1;

    SDL_LockMutex(f->mutex);
    readable_count = f->size - f->rindex_shown;
    if (readable_count > 0) {
        int first_index = -1;
        Frame *last_frame = &f->queue[(f->rindex + f->rindex_shown + readable_count - 1) % f->max_size];
        tail_pts = last_frame->pts;
        serial = last_frame->serial;

        for (i = readable_count - 1; i >= 0; i--) {
            Frame *frame = &f->queue[(f->rindex + f->rindex_shown + i) % f->max_size];
        ...
        if (first_index >= 0) {
            readable_count -= first_index;
            first_pts = f->queue[(f->rindex + f->rindex_shown + first_index) % f->max_size].pts;
            if (!isnan(first_pts) && !isnan(tail_pts)) {
                total_duration = tail_pts - first_pts + last_frame->duration;
            }
        } else {
            readable_count = 0;
            tail_pts = NAN;
    ...
    if (frame_count) {
        *frame_count = readable_count;
    }
    if (duration_sum) {
        *duration_sum = total_duration;
    }
    if (last_frame_pts) {
        *last_frame_pts = tail_pts;
    }
    if (first_frame_pts) {
...
static void decoder_abort(Decoder *d, FrameQueue *fq)
{
    ...
    // decoder_tid may be NULL if initial soft decoder failed
    if (d->decoder_tid) {
...
static void video_image_display2(FFPlayer *ffp)
{
    ...
    if (vp->bmp) {
        ...
        if (ffp->render_wait_start && !ffp->start_on_prepared && is->pause_req) {
            if (!is->first_video_frame_rendered) {
                is->first_video_frame_rendered = 1;
                if (ffp->perf_monitor) perf_monitor_on_first_frame_rendered(ffp->perf_monitor);
                ffp_notify_msg1(ffp, FFP_MSG_VIDEO_RENDERING_START);

                // only when first frame has been rendered,
                // player send prepared message
                if (!is->prepared) {
                    is->prepared = true;
        ...
        SDL_LockMutex(is->seek_info.mutex);
        is->seek_info.last_rendered_pts = vp->pts;
        SDL_UnlockMutex(is->seek_info.mutex);
...
static int fd_io_read(void *fd_io_info, uint8_t *buf, int len) {
    FdIoInfo *info = fd_io_info;
    int to_read = FFMIN(info->length - info->pos, len);
    to_read = FFMAX(to_read, 0);    // should not be negative
    int read_size = read(info->fd, buf, to_read);
    if (read_size < 0) {
         return AVERROR(errno);
    } else if (len > 0 && read_size == 0) {
         return AVERROR_EOF;
    }
    info->pos += read_size;
    return read_size;
...
static int64_t fd_io_seek(void *fd_io_info, int64_t offset, int whence) {
    FdIoInfo *info = fd_io_info;
    if (whence == AVSEEK_SIZE || whence == AVSEEK_CPSIZE) {
         return info->length;
    }

    // if seek pos out of range, return
    if (whence == SEEK_SET && (offset < 0 || offset > info->length) ||
        whence == SEEK_CUR && (offset + info->pos < 0 || offset + info->pos > info->length) ||
        whence == SEEK_END && (offset > 0 || info->length + offset < 0)) {
        return AVERROR(EINVAL);
    }

    int64_t seek_pos = offset;
    if (whence == SEEK_SET) {
         seek_pos += info->offset;
    } else if (whence == SEEK_END) {
         seek_pos = info->offset + info->length - offset;
         whence = SEEK_SET;
    }

    int64_t ret = lseek(info->fd, seek_pos, whence);
    if (ret >= 0) {
         info->pos = ret - info->offset;
    }
    return ret < 0 ? AVERROR(errno) : info->pos;
}

int init_avformat_context(AVFormatContext **a_ic, VideoState *is, FFPlayer *ffp) {
    AVFormatContext *ic = *a_ic;
    const char *fd_offset_length = NULL;
    if (av_strstart(is->filename, "fd_offset_length:", &fd_offset_length)) {
        int fd = atoi(fd_offset_length);
        fd_offset_length = strchr(fd_offset_length, '/') + 1;
        char *tmp = NULL;
        int64_t offset = strtoll(fd_offset_length, &tmp, 10);
        tmp += 1;
        int64_t length = strtoll(tmp, NULL, 10);

        if (offset >= 0 && lseek(fd, offset, SEEK_SET) < 0) {
            av_log(NULL, AV_LOG_ERROR, "seek failed, error: %d", errno);
            return AVERROR(errno);
        }

        void *io_buf = av_malloc(BUFSIZ);
        if (io_buf == NULL) {
            av_log(NULL, AV_LOG_ERROR, "couldn't alloc buffer for aviocontext");
            return AVERROR(ENOMEM);
        }

        FdIoInfo *info = (FdIoInfo *) av_malloc(sizeof(*info));
        if (info == NULL) {
            av_log(NULL, AV_LOG_ERROR, "couldn't alloc memory for FdIoInfo");
            av_freep(&io_buf);
            return AVERROR(ENOMEM);
        }
        info->fd = fd, info->offset = offset, info->length = length, info->pos = 0;
        AVIOContext *io_ctx = avio_alloc_context(io_buf, BUFSIZ, 0, info, fd_io_read, NULL, fd_io_seek);
        if (io_ctx == NULL) {
            av_log(NULL, AV_LOG_ERROR, "couldn't create av_io_context");
            av_freep(&info);
            av_freep(&io_buf);
            return AVERROR(ENOMEM);
        }
        ic->pb = io_ctx;
        is->io_data_opaque = info;
        int ret = avformat_open_input(a_ic, NULL, is->iformat, &ffp->format_opts);

        if (ret != 0) {
            // the avformat_open_input will replace or free the io_buf set into io_ctx
            io_buf = io_ctx->buffer;
            avio_context_free(&io_ctx);
            av_freep(&info);
            av_freep(&io_buf);
            is->io_data_opaque = NULL;
        }

        return ret;
    }
    return avformat_open_input(a_ic, is->filename, is->iformat, &ffp->format_opts);
}

void destroy_avformat_context(AVFormatContext **a_ic, VideoState *is) {
    AVFormatContext *ic = *a_ic;
    // when avformat_open_input failed, the ic will be NULL
    if (ic == NULL) {
         CYLOGI("AVFormatContext is NULL");
         return;
    }

    const char *tmp = NULL;
    if (av_strstart(is->filename, "fd_offset_length:", &tmp)) {
         AVIOContext *io_ctx = ic->pb;
         avformat_close_input(a_ic);
         void *buf = io_ctx->buffer;
         avio_context_free(&io_ctx);
         av_freep(&buf);
         av_freep(&is->io_data_opaque);
         return;
    }
    avformat_close_input(a_ic);
}
...
static void stream_close(FFPlayer *ffp)
{
    ...
    destroy_avformat_context(&is->ic, is);
    ...
    SDL_DestroyMutexP(&is->seek_info.mutex);
...
static double get_clock(Clock *c)
{
    ...
    if (c->paused) {
        ...
        c->last_get_time = c->pts_drift + time - (time - c->last_updated) * (1.0 - c->speed);
    }
    return c->last_get_time;
}

static void set_clock_at(Clock *c, double pts, int serial, double time)
{
    c->last_get_time = c->pts = pts;
    c->last_updated = time;
...
static bool is_pkt_in_playback_range(FFPlayer *ffp, int64_t pkt_ts, int stream_index) {
    VideoState *is = ffp->is;
    AVFormatContext *ic = is->ic;

    int64_t stream_start_time = ic->streams[stream_index]->start_time;

    return ffp->duration == AV_NOPTS_VALUE ||
        (pkt_ts - (stream_start_time != AV_NOPTS_VALUE ? stream_start_time : 0)) *
        av_q2d(ic->streams[stream_index]->time_base) -
...
retry:
        if (frame_queue_nb_remaining(&is->pictq) == 0) {
            int64_t need_quickly_skip_frame;
            SDL_LockMutex(is->seek_info.mutex);
            need_quickly_skip_frame = is->seek_info.need_quickly_skip_frame;
            SDL_UnlockMutex(is->seek_info.mutex);
            // nothing to do, no picture to display in the queue
            if (need_quickly_skip_frame) {
                *remaining_time = 0.001;
            }
        } else {
            double last_duration, duration, delay;
            Frame *vp, *lastvp;
            FFSeekMode curr_seek_mode;
            double seek_pos;
            ...
            bool vp_in_play_range = true;
            if (!isnan(vp->pts)) {
                int64_t vp_pts_i = (int64_t)(vp->pts / av_q2d(is->ic->streams[is->video_stream]->time_base));
                vp_in_play_range = is_pkt_in_playback_range(ffp, vp_pts_i, is->video_stream);
            }

            SDL_LockMutex(is->seek_info.mutex);
            curr_seek_mode = is->seek_info.curr_seek_mode;
            seek_pos = is->seek_info.seek_pos;
            SDL_UnlockMutex(is->seek_info.mutex);
            if (vp->serial != is->videoq.serial || curr_seek_mode == FF_SEEK_MODE_ACCURATE && !isnan(vp->pts) && vp->pts < seek_pos || !vp_in_play_range) {
                frame_queue_next(&is->pictq);
                SDL_LockMutex(is->seek_info.mutex);
                is->seek_info.need_quickly_skip_frame = true;
                SDL_UnlockMutex(is->seek_info.mutex);
                // we don't consider drop frame here
                // because drop frame is neccessary here
                // if (ffp->perf_monitor) perf_monitor_on_video_drop_frame(ffp->perf_monitor);
                goto retry;
            }

            SDL_LockMutex(is->seek_info.mutex);
            is->seek_info.need_quickly_skip_frame = false;
            SDL_UnlockMutex(is->seek_info.mutex);
...
display:
        ...
        if (!ffp->display_disable && is->force_refresh && is->show_mode == SHOW_MODE_VIDEO && is->pictq.rindex_shown) {
            video_display2(ffp);
            if (ffp->perf_monitor) perf_monitor_on_video_render_frame(ffp->perf_monitor);
        }
    }
    is->force_refresh = 0;
    if (ffp->show_status) {
...
static int queue_picture(FFPlayer *ffp, AVFrame *src_frame, double pts, double duration, int64_t pos, int serial)
{
    ...
    if (vp->bmp) {
        ...
        if (!is->viddec.first_frame_decoded) {
            CYLOGD("Video: first frame decoded\n");
...
reload:
    do {
        ...
        seek_pos = is->seek_info.seek_pos;
        SDL_UnlockMutex(is->seek_info.mutex);
    } while (af->serial != is->audioq.serial || curr_seek_mode == FF_SEEK_MODE_ACCURATE && !isnan(af->pts) && af->pts < seek_pos);
    ...
    if (!is->auddec.first_frame_decoded) {
        CYLOGD("avcodec/Audio: first frame decoded\n");
...
static void sdl_audio_callback(void *opaque, Uint8 *stream, int len)
{
    ...
    int64_t audio_flush_timestamp;
    SDL_LockMutex(is->seek_info.mutex);
    audio_flush_timestamp = is->seek_info.audio_flush_timestamp;
    SDL_UnlockMutex(is->seek_info.mutex);

    SDL_LockMutex(ffp->stereo_volume_mutex);
    if (ffp->stereo_volume_changed && (av_gettime_relative() - audio_flush_timestamp > 200000 ||
                                       ffp->user_req_set_stereo_volume || is->ic->duration < AV_TIME_BASE)) {

        ffp->stereo_volume_changed = false;
        ffp->user_req_set_stereo_volume = false;
        SDL_AoutSetStereoVolume(ffp->aout, ffp->stereo_volume[0], ffp->stereo_volume[1]);
        CYLOGI("audio callback set volume: %f, %f", ffp->stereo_volume[0], ffp->stereo_volume[1]);
    }
    SDL_UnlockMutex(ffp->stereo_volume_mutex);

    if(ffp->enable_spatial_audio_changed) {
        ffp->enable_spatial_audio_changed = 0;
        SDL_AoutSetSpatialAudioEnable(ffp->aout, ffp->enable_spatial_audio);
    }

    if(ffp->entity_id_changed) {
        ffp->entity_id_changed = 0;
        SDL_AoutAttachToEntity(ffp->aout, ffp->entity_id);
    }

    if(ffp->audio_bind_id_changed) {
        ffp->audio_bind_id_changed = 0;
        SDL_AoutAttachToContainer(ffp->aout, ffp->audio_bind_id);
    }
    ...
    while (len > 0) {
        ...
        if (is->auddec.pkt_serial != is->audioq.serial) {
            ...
            SDL_LockMutex(is->seek_info.mutex);
            is->seek_info.audio_flush_timestamp = av_gettime_relative();
            SDL_UnlockMutex(is->seek_info.mutex);
            SDL_AoutSetStereoVolume(ffp->aout, 0, 0);
            CYLOGI("audio callback flush audio, set volume to 0");

            SDL_LockMutex(ffp->stereo_volume_mutex);
            ffp->stereo_volume_changed = true;
            SDL_UnlockMutex(ffp->stereo_volume_mutex);
            break;
        }
    ...
    if (!isnan(is->audio_clock)) {
        // CYLOGI("audio call back set clock: %lf, %lf, %lf, %ld, %ld", is->audio_clock, (double)(is->audio_write_buf_size) / is->audio_tgt.bytes_per_sec, SDL_AoutGetLatencySeconds(ffp->aout), ffp->audio_callback_time, av_gettime_relative());
        set_clock_at(&is->audclk, is->audio_clock - (double)(is->audio_write_buf_size) / is->audio_tgt.bytes_per_sec - SDL_AoutGetLatencySeconds(ffp->aout), is->audio_clock_serial, av_gettime_relative() / 1000000.0);
        sync_clock_to_slave(&is->extclk, &is->audclk);
    }
    if (!is->first_audio_frame_rendered) {
        is->first_audio_frame_rendered = 1;
...
static int audio_open(FFPlayer *opaque, int64_t wanted_channel_layout, int wanted_nb_channels, int wanted_sample_rate, struct AudioParams *audio_hw_params)
{
    ...
    AVDictionary **dict = ffp_get_opt_dict(ffp, 4);
    int64_t value = 0;
    AVDictionaryEntry *entry = av_dict_get(*dict, "spatial_audio_mode", NULL, 0);
    CYLOGI("spatial_audio_mode entry: %p", entry);
    if (entry) {
        value = strtol(entry->value, NULL, 10);
        wanted_spec.spatial_audio_mode = value;
        av_log(NULL, AV_LOG_WARNING, "find entry for spatial_audio_mode %lld", value);
    } else {
        wanted_spec.spatial_audio_mode = 2; // TODO: auto
    }
//    entry = av_dict_get(*dict, "enable_spatial_audio", NULL, 0);
//    if (entry) {
//        value = strtol(entry->value, NULL, 10);
//        wanted_spec.enable_spatial_audio = (value != 0);
//        av_log(NULL, AV_LOG_WARNING, "find entry for enable_spatial_audio %lld", value);
//    } else {
//        wanted_spec.enable_spatial_audio = true;
//    }
//    entry = av_dict_get(*dict, "spatial_audio_entity_id", NULL, 0);
//    if (entry) {
//        value = strtol(entry->value, NULL, 10);
//        wanted_spec.spatial_audio_entity_id = value;
//        av_log(NULL, AV_LOG_WARNING, "find entry for spatial_audio_entity_id %lld", value);
//    } else {
//        wanted_spec.spatial_audio_entity_id = -1;
//    }
//    entry = av_dict_get(*dict, "spatial_audio_audio_bind_id", NULL, 0);
//    if (entry) {
//        value = strtol(entry->value, NULL, 10);
//        wanted_spec.spatial_audio_audio_bind_id = value;
//        av_log(NULL, AV_LOG_WARNING, "find entry for spatial_audio_audio_bind_id %lld", value);
//    } else {
//        wanted_spec.spatial_audio_audio_bind_id = -1;
//    }
...
static int stream_component_open(FFPlayer *ffp, int stream_index)
{
    ...
    if (!avctx) {
        ffp_notify_msg2(ffp, FFP_MSG_ERROR, CYPRESS_PLAYER_ERROR_NOMEM);
        return AVERROR(ENOMEM);
    }

    ret = avcodec_parameters_to_context(avctx, ic->streams[stream_index]->codecpar);
    if (ret < 0) {
        av_log(NULL, AV_LOG_ERROR, "avcodec_parameters_to_context failed, %s(%d)", av_err2str(ret), ret);
        last_error = CYPRESS_PLAYER_ERROR_DECODER;
        goto fail;
    }
    ...
    if (!codec) {
        ...
        ret = AVERROR(EINVAL);
        // TODO: when audio and video codecs are not found, we can report error.
        // last_error = CYPRESS_PLAYER_ERROR_DECODER_NOT_FOUND;
        goto fail;
    }
    ...
    case AVMEDIA_TYPE_AUDIO:
        ...
        if ((ret = audio_open(ffp, channel_layout, nb_channels, sample_rate, &is->audio_tgt)) < 0) {
            last_error = CYPRESS_PLAYER_ERROR_AUDIO_OPEN_FAILED;
            goto fail;
        }
        ...
        if ((ret = avcodec_open2(avctx, codec, &opts)) < 0) {
            last_error = CYPRESS_PLAYER_ERROR_DECODER_INIT_FAILED;
            goto fail;
        }
        if (ffp->perf_monitor) perf_monitor_on_audio_codec_init_done(ffp->perf_monitor);
        ...
        if ((ret = decoder_start(&is->auddec, audio_thread, ffp, "ff_audio_dec")) < 0) {
            last_error = CYPRESS_PLAYER_ERROR_AUDIO;
            goto out;
         }
    ...
    case AVMEDIA_TYPE_VIDEO:
        ...
        if (ffp->async_init_decoder) {
            ...
            if (ret || !ffp->node_vdec) {
                ...
                if (!ffp->node_vdec) {
                    av_log(NULL, AV_LOG_ERROR, "couldn't open video decoder 1");
                    last_error = CYPRESS_PLAYER_ERROR_DECODER_INIT_FAILED;
                    goto out;
                }
            }
            ...
            ffp->node_vdec = ffpipeline_open_video_decoder(ffp->pipeline, ffp);
            if (!ffp->node_vdec) {
                av_log(NULL, AV_LOG_ERROR, "couldn't open video decoder 2");
                last_error = CYPRESS_PLAYER_ERROR_DECODER_INIT_FAILED;
                goto out;
            }
            if (!ffp->node_vdec->hardware && (
                    (ret = res_mgr_reset(ffp->res_mgr)) != 0 ||
                    (ret = avcodec_open2(avctx, codec, &opts)) < 0
                )) {
                av_log(NULL, AV_LOG_ERROR, "couldn't open soft decoder or reset res_mgr failed");
                last_error = CYPRESS_PLAYER_ERROR_DECODER_INIT_FAILED;
                goto out;
            }
            if (ffp->perf_monitor) perf_monitor_on_video_codec_init_done(ffp->perf_monitor);
        }
        if ((ret = decoder_start(&is->viddec, video_thread, ffp, "ff_video_dec")) < 0) {
            av_log(NULL, AV_LOG_ERROR, "couldn't start decoder for video, ret: %d", ret);
            last_error = CYPRESS_PLAYER_ERROR_VIDEO;
            goto out;
        }
    ...
    case AVMEDIA_TYPE_SUBTITLE:
        ...
        if ((ret = decoder_start(&is->subdec, subtitle_thread, ffp, "ff_subtitle_dec")) < 0) {
            av_log(NULL, AV_LOG_ERROR, "couldn't' start subtitle decoder, ret: %d", ret);
            last_error = CYPRESS_PLAYER_ERROR_SUBTITLE;
            goto out;
        }
        break;
    default:
...
out:
    if (last_error) {
        av_log(NULL, AV_LOG_ERROR, "open stream component error: %d", last_error);
...
static bool decide_seek_mode_policy_gop(FFPlayer *ffp) {
    VideoState *is = ffp->is;

    double gop_duration = is->gop_info.gop_duration;
    double total_duration = is->ic->duration * av_q2d(AV_TIME_BASE_Q);

    // if gop_duration is short or the video duration is too short
    if (gop_duration < 1.5 || is->video_st && fabs(gop_duration - (double)INT_MAX) > 1.0 && gop_duration / total_duration > 0.2) {
        return true;
    }
...
static bool decide_seek_mode_policy_bitrate(FFPlayer *ffp) {
    VideoState *is = ffp->is;
    ...
    int width = is->video_st->codecpar->width;
    int height = is->video_st->codecpar->height;
    int bitrate = is->video_st->codecpar->bit_rate;

    if (width * height <= 854 * 480) {
        return true;
    }

    if (width * height <= 1920 * 1080 && bitrate < 3 * 1024 *1024) {
        return true;
    }

    if (width * height <= 3840 * 2160 && bitrate < 2 * 1024 *1024) {
        return true;
    }
...
static bool decide_seek_mode_policy_frame_gap(FFPlayer *ffp) {
    VideoState *is = ffp->is;

    double frame_gap = is->gop_info.max_frame_gap;
...
static void decide_seek_mode_policy(FFPlayer *ffp) {
    static bool (*decide_seek_mode_policy_func[])(FFPlayer *ffp) = {
        decide_seek_mode_policy_gop,
        decide_seek_mode_policy_bitrate,
        decide_seek_mode_policy_frame_gap
    };
    static size_t decide_seek_mode_policy_func_size = sizeof(decide_seek_mode_policy_func) / sizeof(*decide_seek_mode_policy_func);

    VideoState *is = ffp->is;
    FFSeekMode last_mode;
    if (ffp->user_req_seek_mode != FF_SEEK_MODE_AUTO) {
        // we need to override it to prevent cocurrent issue
        SDL_LockMutex(is->seek_info.mutex);
        is->seek_info.curr_seek_mode = ffp->user_req_seek_mode;
        SDL_UnlockMutex(is->seek_info.mutex);
        return;
    }
    SDL_LockMutex(is->seek_info.mutex);
    last_mode = is->seek_info.curr_seek_mode;
    SDL_UnlockMutex(is->seek_info.mutex);

    bool enable_accurate_mode = false;

    for (size_t i = 0; !enable_accurate_mode && i < decide_seek_mode_policy_func_size; i++) {
        enable_accurate_mode = enable_accurate_mode || decide_seek_mode_policy_func[i](ffp);
    }

    SDL_LockMutex(is->seek_info.mutex);
    if (enable_accurate_mode) {
        is->seek_info.curr_seek_mode = FF_SEEK_MODE_ACCURATE;
    } else {
        is->seek_info.curr_seek_mode = FF_SEEK_MODE_PREVIOUS;
    }
    if (last_mode != is->seek_info.curr_seek_mode) {
        av_log(NULL, AV_LOG_INFO, "seek mode change from %d to %d", last_mode, is->seek_info.curr_seek_mode);
    }
    SDL_UnlockMutex(is->seek_info.mutex);
}

static const char *codec_id2mime(int codec_id) {
    switch (codec_id) {
        //audio
        ...
        case AV_CODEC_ID_MSMPEG4V1:
            return "video/msmpeg1";
        case AV_CODEC_ID_MSMPEG4V2:
            return "video/msmpeg2";
        case AV_CODEC_ID_MSMPEG4V3:
            return "video/msmpeg3";
        ...
        case AV_CODEC_ID_METT_DISPARITY:
            return "application/disparity";
        default: {
...
static int read_thread(void *arg)
{
    ...
    CYLOGI("read thread [%d]", gettid());
    if (!wait_mutex) {
        av_log(NULL, AV_LOG_FATAL, "SDL_CreateMutex(): %s\n", SDL_GetError());
        ret = AVERROR(ENOMEM);
        last_error = CYPRESS_PLAYER_ERROR_UNKNOWN;
        goto fail;
    }
    ...
    if (!ic) {
        ...
        ret = AVERROR(ENOMEM);
        last_error = CYPRESS_PLAYER_ERROR_NOMEM;
        goto fail;
    }
    ...
    err = init_avformat_context(&ic, is, ffp);
    // err = avformat_open_input(&ic, is->filename, is->iformat, &ffp->format_opts);

    if (err < 0) {
        print_error(is->filename, err);
        last_error = ff_err2cypress_err(err);
        ret = -1;
        goto fail;
    }
    ffp_notify_msg1(ffp, FFP_MSG_OPEN_INPUT);
    if (ffp->perf_monitor) perf_monitor_set_media_duration(ffp->perf_monitor, (int32_t)(ic->duration * av_q2d(AV_TIME_BASE_Q)));
    ...
    bool need_find_stream_info = ffp->find_stream_info;
    if (!need_find_stream_info) {
        // we need stream information for formats except mov
        need_find_stream_info = (0 != strcmp(ic->iformat->name, "mov,mp4,m4a,3gp,3g2,mj2"));
    }
    if (!need_find_stream_info) {
         for (int si = 0; si < ic->nb_streams; si++) {
            // for av1, we need steam info
            if (ic->streams[si]->codecpar->codec_id == AV_CODEC_ID_AV1) {
                need_find_stream_info = true;
                break;
            }
         }
    }
    if (need_find_stream_info) {
        ...
        if (err < 0) {
            ...
            last_error = CYPRESS_PLAYER_ERROR_STREAM_INFO_NOT_FOUND;
            ret = -1;
            goto fail;
        }
    }
    ...
    av_log(NULL, AV_LOG_INFO, "input format: %s", ic->iformat->name);

    int video_stream_count = 0;
    int h264_stream_count = 0;
    int first_h264_stream = -1;
    int audio_stream_count = 0;
    int audio_stream_index[255];
    ...
    for (i = 0; i < ic->nb_streams; i++) {
        ...
        if (type == AVMEDIA_TYPE_VIDEO) {
            ...
            audio_stream_count++;
            audio_stream_index[audio_stream_count - 1] = i;
        }
    }
    ...
    if (!ffp->audio_disable)
        st_index[AVMEDIA_TYPE_AUDIO] =
            av_find_best_stream(ic, AVMEDIA_TYPE_AUDIO,
                                ...
                                st_index[AVMEDIA_TYPE_VIDEO],
                                NULL, 0);
        CYLOGI("find best audio stream %d\n", st_index[AVMEDIA_TYPE_AUDIO]);
    ...
    if (st_index[AVMEDIA_TYPE_AUDIO] >= 0) {
        for(int i = 0 ; i < audio_stream_count ; i++) {
            CYLOGI("open audio stream %d\n", audio_stream_index[i]);
            if(stream_component_open(ffp, audio_stream_index[i]) == 0){
                st_index[AVMEDIA_TYPE_AUDIO] = audio_stream_index[i];
                int stream_index = audio_stream_index[i];
                if (ffp->perf_monitor) {
                    perf_monitor_set_audio_mime(ffp->perf_monitor, codec_id2mime(ic->streams[stream_index]->codecpar->codec_id));
                    perf_monitor_set_audio_sample_rate(ffp->perf_monitor, ic->streams[stream_index]->codecpar->sample_rate);
                    perf_monitor_set_audio_channel_count(ffp->perf_monitor, ic->streams[stream_index]->codecpar->channels);
                    perf_monitor_set_audio_bitrate(ffp->perf_monitor, ic->streams[stream_index]->codecpar->bit_rate);
                }
                break;
            }
        }
    ...
    ret = -1;
    if (st_index[AVMEDIA_TYPE_VIDEO] >= 0) {
        int stream_index = st_index[AVMEDIA_TYPE_VIDEO];
        if (ffp->perf_monitor) {
            perf_monitor_set_video_mime(ffp->perf_monitor, codec_id2mime(ic->streams[stream_index]->codecpar->codec_id));
            perf_monitor_set_video_resolution(ffp->perf_monitor, ic->streams[stream_index]->codecpar->width, ic->streams[stream_index]->codecpar->height);
            perf_monitor_set_video_fps(ffp->perf_monitor, (int32_t)av_q2d(av_guess_frame_rate(ic, ic->streams[stream_index], NULL)));
            perf_monitor_set_video_bitrate(ffp->perf_monitor, ic->streams[stream_index]->codecpar->bit_rate);
        }
    ...
    if (is->video_stream < 0 && is->audio_stream < 0) {
        av_log(NULL, AV_LOG_FATAL, "Failed to open file '%s' or configure filtergraph\n",
               is->filename);
        last_error = CYPRESS_PLAYER_ERROR_STREAM_NO_PLAYABLE_AV;
        ret = -1;
        goto fail;
    }
    if (is->audio_stream >= 0) {
    ...
    if (is->video_st && is->video_st->codecpar) {
        ...
        int width = rotate % 180 == 0 ? codecpar->width : codecpar->height;
        int height = rotate % 180 == 0 ? codecpar->height : codecpar->width;
    ...
    // if the media doesn't have video stream,
    // player sends prepared message, otherwise
    // player sends prepared message when first frame rendered.
    // NOTE: the player has been prepared here
    if (!is->video_st) {
        is->prepared = true;
        ffp_notify_msg1(ffp, FFP_MSG_PREPARED);
    }
    ...
    if (ffp->seek_at_start != AV_NOPTS_VALUE) {
        ffp_seek_to_l(ffp, (long)(ffp->seek_at_start));
    }

    // record the last video pts to calculate the last frame's duration
    double last_video_pts = (double)INT_MIN;
    double last_i_frame_pts = (double)INT_MIN;
...
//      of the seek_pos/seek_rel variables
            ...
            bool skip_avformat_seek = false;
            FFSeekMode curr_seek_mode;
            int frame_count = 0;
            double first_frame_pts = NAN;
            double last_frame_pts = NAN;
            double frame_duration = 0;
            double remaing_duration = 0;
            int last_serial = -1;
            ...
            do {
                ...
                SDL_UnlockMutex(is->seek_info.mutex);
                if (completed) {
                    CYLOGI("couldn't skip avformat seek because playback completed");
                    break;
                }
                if (curr_seek_mode != FF_SEEK_MODE_ACCURATE) {
                    CYLOGI("couldn't skip avformat seek because seek mode is not accurate");
                    break;
                }
                if (!is->video_st) {
                    CYLOGI("couldn't skip avformat seek because video stream is null");
                    break;
                }

                frame_queue_get_snapshot(is, &is->pictq, &frame_count, &remaing_duration, &last_frame_pts, &first_frame_pts, &last_serial);

                if (frame_count <= 0) {
                    CYLOGI("couldn't skip avformat seek because frame count in queue <= 0");
                    break;
                }

                if (last_serial == -1 || last_serial != is->videoq.serial) {
                    CYLOGI("couldn't skip avformat seek because last serial is not videoq serial");
                    break;
                }

                if (isnan(last_frame_pts) || isnan(first_frame_pts)) {
                    CYLOGI("couldn't skip avformat seek because first/last frame pts is invalid");
                    break;
                }

                double seek_target_pts = fftime_to_milliseconds(seek_target) / 1000.0;

                if (seek_target_pts < first_frame_pts) {
                    CYLOGI("couldn't skip avformat seek because seek target is before first frame pts, seek target: %lf, first frame pts: %lf", seek_target_pts, first_frame_pts);
                    break;
                }

                if (seek_target_pts <= last_frame_pts) {
                    skip_avformat_seek = true;
                    CYLOGI("enable skip avformat seek because seek target is in frame queue");
                    break;
                }

                int64_t last_frame_ts = av_rescale_q((int64_t)(last_frame_pts * AV_TIME_BASE), AV_TIME_BASE_Q, is->video_st->time_base);
                int64_t seek_target_ts = av_rescale_q(seek_target, AV_TIME_BASE_Q, is->video_st->time_base);

                int index1 = av_index_search_timestamp(is->video_st, last_frame_ts, AVSEEK_FLAG_BACKWARD);
                int index2 = av_index_search_timestamp(is->video_st, seek_target_ts, AVSEEK_FLAG_BACKWARD);

                if (index1 >= 0) index1 -= is->video_st->index_entries[index1].min_distance;
                if (index2 >= 0) index2 -= is->video_st->index_entries[index2].min_distance;

                bool in_same_gop = (index1 >= 0 && index2 >= 0 && index1 == index2);

                if (in_same_gop) {
                    skip_avformat_seek = true;
                    CYLOGI("enable skip avformat seek because in same gop");
                    break;
                }

                frame_duration = remaing_duration / frame_count;
                if (frame_duration > 0 && frame_duration <= 0.1 && (seek_target_pts - last_frame_pts) / frame_duration * 0.004 <= 0.1) {
                    skip_avformat_seek = true;
                    CYLOGI("enable skip avformat seek because seek target is near current rendered pts");
                    break;
                }
            } while (0);

            CYLOGI("skip_avformat_seek: %d", skip_avformat_seek);

            if (!skip_avformat_seek) {
                ...
                if (ret < 0) {
                    ...
                    if (is->audio_stream >= 0) {
                        ...
                        // EOF restart may have no stale PCM left in callback, so do not rely on serial mismatch to flush Aout.
                        // to ensure that the audio latency estimator is updated and not used the old data.
                        if (completed)
                            SDL_AoutFlushAudio(ffp->aout);
                    }
                    if (is->subtitle_stream >= 0) {
            ...
            SDL_LockMutex(is->seek_info.mutex);
            is->seek_info.seek_pos = fftime_to_milliseconds(seek_target) / 1000.0;
            SDL_UnlockMutex(is->seek_info.mutex);
            is->seek_req = 0;

            if (!skip_avformat_seek) {
                // the last_i_frame_pts is invalid when avformat seek was done
                last_i_frame_pts = (double)INT_MIN;
                last_video_pts = (double)INT_MIN;
                is->eof = 0;
                is->queue_attachments_req = 1;
            }
            // we want to receive first frame rendered notification if the surface is not null
            if (ffpipeline_get_native_window(ffp->pipeline) != NULL) {
                is->first_video_frame_rendered = 0;
            }
#ifdef FFP_MERGE
            if (is->paused)
        ...
        if ((!is->paused || completed) &&
            ...
            if (ffp->loop != 1 && (!ffp->loop || --ffp->loop)) {
                ...
                // the seek_pos should be reset when auto loop
                SDL_LockMutex(is->seek_info.mutex);
                is->seek_info.seek_pos = (double)INT_MIN;
                SDL_UnlockMutex(is->seek_info.mutex);
                ...
                if (completed) {
                    ...
                    int64_t audio_duration = is->audio_st ?
                        av_rescale_q(is->audio_st->duration + (is->audio_st->start_time != AV_NOPTS_VALUE ? is->audio_st->start_time : 0),
                                    is->audio_st->time_base, AV_TIME_BASE_Q) : 0;
                    int64_t video_duration = is->video_st ?
                        av_rescale_q(is->video_st->duration + (is->video_st->start_time != AV_NOPTS_VALUE ? is->video_st->start_time : 0),
                                    is->video_st->time_base, AV_TIME_BASE_Q) : 0;
                    int64_t audio_latency = (is->audio_st ? SDL_AoutGetLatencySeconds(ffp->aout) * 1000000 : 0);
                    int64_t last_frame_pts_us = (int64_t)(FFMAX(last_video_pts, 0) * 1000000);
                    // may be not reasonable, may ignore short audio only media
                    int64_t last_audio_pts_us = FFMAX(audio_duration - audio_latency, 0);

                    int64_t total_delay = FFMAX(
                        FFMAX(audio_duration, video_duration) - FFMAX(last_frame_pts_us, last_audio_pts_us),
                        0
                    );

                    total_delay = (int64_t)(total_delay / ffp->pf_playback_rate) / 1000;

                    CYLOGI("total_delay: %lld ms last_frame_pts_us: %lld last_audio_pts_us: %lld audio_duration: %lld video_duration: %lld audio_latency: %lld", total_delay, last_frame_pts_us, last_audio_pts_us, audio_duration, video_duration, audio_latency);

                    while (!is->abort_request && total_delay > 0 && !is->seek_req) {
                        SDL_LockMutex(wait_mutex);
                        SDL_CondWaitTimeout(is->continue_read_thread, wait_mutex, FFMIN(total_delay, 10));
                        SDL_UnlockMutex(wait_mutex);
                        total_delay -= 10;
                    }

                    CYLOGI("delay complete");

                    if (is->abort_request || is->seek_req) {
                        continue;
                    }
                    ...
                    if (ffp->error) {
                        ...
                        ffp_notify_msg2(ffp, FFP_MSG_ERROR, CYPRESS_PLAYER_ERROR_DEMUXER_READ_FAILED);
                    } else {
        ...
        pkt_in_play_range = is_pkt_in_playback_range(ffp, pkt_ts, pkt->stream_index);
        if (!pkt_in_play_range && pkt->stream_index == is->video_stream && fabs(last_i_frame_pts - (double)INT_MIN) > 0.0001) {
            int64_t l_i_pts = (int64_t)(last_i_frame_pts / av_q2d(ic->streams[pkt->stream_index]->time_base));
            bool last_i_frame_in_play_range = is_pkt_in_playback_range(ffp, l_i_pts, pkt->stream_index);
            pkt_in_play_range = last_i_frame_in_play_range;
        }
        if (pkt->stream_index == is->audio_stream && pkt_in_play_range) {
            packet_queue_put(&is->audioq, pkt);
        } else if (pkt->stream_index == is->video_stream && pkt_in_play_range
                   && !(is->video_st && (is->video_st->disposition & AV_DISPOSITION_ATTACHED_PIC))) {
            double curr_pts = pkt_ts * av_q2d(ic->streams[pkt->stream_index]->time_base);
            if (fabs(last_video_pts - (double)INT_MIN) > 0.0001) {
                double frame_gap = curr_pts - last_video_pts;
                frame_gap = FFMAX(frame_gap, 0);
                is->gop_info.max_frame_gap = FFMAX(is->gop_info.max_frame_gap, frame_gap);
            }
            last_video_pts = FFMAX(last_video_pts, curr_pts);
            if (pkt->flags & AV_PKT_FLAG_KEY) {
                if (fabs(last_i_frame_pts - (double)INT_MIN) < 0.0001) {
                    last_i_frame_pts = curr_pts;
                } else {
                    double new_gop_duration = curr_pts - last_i_frame_pts;
                    last_i_frame_pts = curr_pts;
                    if (fabs(is->gop_info.gop_duration - (double)INT_MAX) < 0.0001) {
                        is->gop_info.gop_duration = new_gop_duration;
                    } else {
                        is->gop_info.gop_duration = is->gop_info.gop_duration * 0.7 + new_gop_duration * 0.3;
                    }
                }
                decide_seek_mode_policy(ffp);
            }
        ...
        if (ffp->ijkmeta_delay_init && !init_ijkmeta &&
                (is->first_video_frame_rendered || !is->video_st) && (is->first_audio_frame_rendered || !is->audio_st)) {
            ijkmeta_set_avformat_context_l(ffp->meta, ic);
            init_ijkmeta = 1;
        }
        ...
        if (ffp->packet_buffering) {
            ...
            if ((!is->first_video_frame_rendered && is->video_st) || (!is->first_audio_frame_rendered && is->audio_st)) {
 ...
 fail:
    if (ic && !is->ic)
        destroy_avformat_context(&ic, is);
        // avformat_close_input(&ic);

    // sdk does not check if prepared or abort_request but simply checks last_error
    // user may release player in PREPARING STATE at which last_error is 0
    if ((ffp->last_error = last_error) != 0) {
        ffp_notify_msg2(ffp, FFP_MSG_ERROR, last_error);
    }
...
static VideoState *stream_open(FFPlayer *ffp, const char *filename, AVInputFormat *iformat)
{
    ...
    is->seek_info.mutex = SDL_CreateMutex();
    is->seek_info.seek_pos = (double)INT_MIN;
    is->seek_info.curr_seek_mode = ffp->user_req_seek_mode;
    is->seek_info.audio_flush_timestamp = 0;
    is->seek_info.need_quickly_skip_frame = 0;
    is->seek_info.last_rendered_pts = (double)INT_MIN;
    is->gop_info.gop_duration = (double)INT_MAX;
    is->gop_info.max_frame_gap = 0;
...
static int video_refresh_thread(void *arg)
{
    ...
    CYLOGI("video refresh thread [%d]", gettid());
...
{
    int ijk_level = NC_LOG_LEVEL_V;
    if      (av_level <= AV_LOG_PANIC)      ijk_level = NC_LOG_LEVEL_E;
    else if (av_level <= AV_LOG_FATAL)      ijk_level = NC_LOG_LEVEL_E;
    else if (av_level <= AV_LOG_ERROR)      ijk_level = NC_LOG_LEVEL_E;
    else if (av_level <= AV_LOG_WARNING)    ijk_level = NC_LOG_LEVEL_W;
    else if (av_level <= AV_LOG_INFO)       ijk_level = NC_LOG_LEVEL_I;
    // AV_LOG_VERBOSE means detailed info
    else if (av_level <= AV_LOG_VERBOSE)    ijk_level = NC_LOG_LEVEL_I;
    else if (av_level <= AV_LOG_DEBUG)      ijk_level = NC_LOG_LEVEL_D;
    else if (av_level <= AV_LOG_TRACE)      ijk_level = NC_LOG_LEVEL_V;
    else                                    ijk_level = NC_LOG_LEVEL_V;
    return ijk_level;
}
...
inline static int log_level_ijk_to_av(int ijk_level)
{
    ...
    if      (ijk_level <= NC_LOG_LEVEL_E)  av_level = AV_LOG_ERROR;
    else if (ijk_level <= NC_LOG_LEVEL_W)  av_level = AV_LOG_WARNING;
    else if (ijk_level <= NC_LOG_LEVEL_I)  av_level = AV_LOG_INFO;
    else if (ijk_level <= NC_LOG_LEVEL_D)  av_level = AV_LOG_INFO;
    else if (ijk_level <= NC_LOG_LEVEL_V)  av_level = AV_LOG_VERBOSE;
...
void ffp_global_init()
{
    ...
    CYLOGD("ijkmediaplayer version : %s", ijkmp_version());
    ...
    av_init_packet(&set_surface_pkt);
    set_surface_pkt.data = (uint8_t *)&set_surface_pkt;
...
FFPlayer *ffp_create()
{
    ...
    ffp->stereo_volume_mutex = SDL_CreateMutex();
...
void ffp_destroy(FFPlayer *ffp)
{
    ...
    // remember res_mgr and codec_reuse_enable before
    // calling ffp_reset_internal
    ResMgr *res_mgr = ffp->res_mgr;
    int codec_reuse_enable = ffp->codec_reuse_enable;
    ...
    SDL_DestroyMutexP(&ffp->stereo_volume_mutex);

    msg_queue_destroy(&ffp->msg_queue);

    if (!codec_reuse_enable) {
        CYLOGI("disable reuse codec, now reset res_mgr");
        res_mgr_reset(res_mgr);
    }
    av_free(ffp);
}
...
int ffp_prepare_async_l(FFPlayer *ffp, const char *file_name)
{
    ...
    if (ffp->perf_monitor) perf_monitor_on_prepare_start(ffp->perf_monitor);
    ...
    if (!ffp->aout) {
        ...
        if (!ffp->aout)
            return CYPRESS_PLAYER_ERROR_AUDIO_OPEN_FAILED;
    }
    ...
    if (!is) {
        ...
        return CYPRESS_PLAYER_ERROR_STREAM;
    }
...
void ffp_remove_user_req(FFPlayer *ffp) {
    msg_queue_remove(&ffp->msg_queue, FFP_REQ_START);
    msg_queue_remove(&ffp->msg_queue, FFP_REQ_PAUSE);
    msg_queue_remove(&ffp->msg_queue, FFP_REQ_SEEK);
    msg_queue_remove(&ffp->msg_queue, FFP_REQ_STOP);
}
...
int ffp_stop_l(FFPlayer *ffp)
{
    ...
    // NOTE: after calling msg_queue_abort(), msg thread will lost
    // all msg and then exit. However, sdk need state changed msg.
    // msg_queue_abort(&ffp->msg_queue);

    // remove all req message and send STOP msg to exit
    ffp_remove_user_req(ffp);
    msg_queue_put_simple1(&ffp->msg_queue, FFP_REQ_STOP);
...
int ffp_seek_to_l(FFPlayer *ffp, long msec)
{
    ...
    if (ffp->start_time != AV_NOPTS_VALUE && msec * 1000 < ffp->start_time) {
        seek_pos = milliseconds_to_fftime(ffp->start_time / 1000);
    }

    if (ffp->start_time != AV_NOPTS_VALUE && ffp->duration != AV_NOPTS_VALUE) {
        int64_t ab_end_ms = (ffp->duration + ffp->start_time) / 1000;
        int64_t ab_end = milliseconds_to_fftime(ab_end_ms);
        duration = FFMIN(duration, ab_end);
    }

    if (duration > 0 && seek_pos >= duration && ffp->enable_accurate_seek) {
        toggle_pause(ffp, 1);
        ffp_notify_msg1(ffp, FFP_MSG_COMPLETED);
        ffp->last_get_position = 0;
        return 0;
    }
    ...
    if (ffp->perf_monitor) perf_monitor_on_seek_start(ffp->perf_monitor);
...
long ffp_get_current_position_l(FFPlayer *ffp)
{
    ...
    // the pos may exceed the duration
    adjust_pos = FFMIN((long)adjust_pos, ffp_get_ab_end_pos_l(ffp));

    // the pos may rewind
    if (ffp->last_get_position > adjust_pos && ffp->last_get_position <= adjust_pos + 200) {
        adjust_pos = ffp->last_get_position;
    }
    ffp->last_get_position = adjust_pos;

    return adjust_pos;
}

long ffp_get_ab_end_pos_l(FFPlayer *ffp)
{
    assert(ffp);
    VideoState *is = ffp->is;
    if (!is || !is->ic)
    ...
    int64_t ab_start_time = (ffp->start_time == AV_NOPTS_VALUE ? 0 : ffp->start_time / 1000);
    int64_t ab_duration = (ffp->duration == AV_NOPTS_VALUE ? ffp_get_duration_l(ffp) : ffp->duration / 1000);
    int64_t ab_end_time = FFMIN(ab_start_time + ab_duration, ffp_get_duration_l(ffp));
...
bool ffp_is_set_surface_packet(AVPacket *pkt)
{
    if (!pkt)
        return false;

    return pkt->data == set_surface_pkt.data;
}
...
int ffp_get_video_rotate_degrees(FFPlayer *ffp)
{
    ...
    switch (theta) {
        ...
        case 360:
            ...
            break;
        default:
            CYLOGW("Unknown rotate degress: %d\n", theta);
            theta = 0;
            break;
    }
...
void ffp_set_spatial_audio_enable(FFPlayer *ffp, bool enable)
{
    if(ffp->enable_spatial_audio != enable) {
        ffp->enable_spatial_audio = enable;
        ffp->enable_spatial_audio_changed = 1;
    }
}

void ffp_set_spatial_audio_entity(FFPlayer *ffp, int64_t entity_id)
{
    if(ffp->entity_id != entity_id) {
        ffp->entity_id = entity_id;
        ffp->entity_id_changed = 1;
    }
}

void ffp_set_spatial_audio_container(FFPlayer *ffp, int64_t audio_bind_id)
{
    if(ffp->audio_bind_id != audio_bind_id) {
        ffp->audio_bind_id = audio_bind_id;
        ffp->audio_bind_id_changed = 1;
    }
}

void ffp_set_seek_mode(FFPlayer *ffp, FFSeekMode seek_mode)
{
    av_log(NULL, AV_LOG_INFO, "set user_req_seek_mode %d", (int)seek_mode);
    ffp->user_req_seek_mode = seek_mode;
    if (ffp->is && seek_mode != FF_SEEK_MODE_AUTO) {
        SDL_LockMutex(ffp->is->seek_info.mutex);
        ffp->is->seek_info.curr_seek_mode = seek_mode;
        SDL_UnlockMutex(ffp->is->seek_info.mutex);
    }
}

int ffp_set_ab_playback_range(FFPlayer *ffp, int64_t from_ms, int64_t to_ms)
{
    if (from_ms < 0) from_ms = 0;

    if (!ffp || from_ms > to_ms)
        return -1;

    av_log(NULL, AV_LOG_INFO, "set ab_playback_range %" PRIi64 " - %" PRIi64, from_ms, to_ms);

    ffp->start_time = from_ms * 1000;
    ffp->duration = (to_ms - from_ms) * 1000;
...
int ffp_send_set_surface_pkt(FFPlayer *ffp)
{
    if (!ffp)
        return -1;
    ...
    /**
     * send a packet to signal the enqeue thread to setup new surface
     * NOTE: 1. enqueue thread may setup new surface before receives this packet
     *       2. enqueue thread will mark all output buffer invalid after setup new surface
     *       3. actually packet_queue_put should be called in read_thread,
     *          but it will cost some time to wait read_thread send packet
     *       4. as a result, we couldn't know when the codec surface was updated
     *          and what data was sent to codec (codec may receive no-IDR frame after surface updated),
     *          which may lead to frozen. As to resolve this issue, we should seek after set surface.
     */
    int ret = packet_queue_put(&is->videoq, &set_surface_pkt);
    return ret;
}
