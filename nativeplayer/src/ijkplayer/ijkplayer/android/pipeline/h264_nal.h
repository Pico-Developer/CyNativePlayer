                            ...
                            size_t *p_sps_pps_size, size_t *p_nal_size)
{
    ...
    for ( unsigned int j = 0; j < 2; j++ )
    {
        ...
        if( i_data_size < 1 )
        {
            CYLOGE( "PPS too small after processing SPS/PPS %u",
                    i_data_size );
            return -1;
        }
        ...
        for ( unsigned int i = 0; i < i_loop_end; i++)
        {
            if( i_data_size < 2 )
            {
                CYLOGE( "SPS is too small %u", i_data_size );
                return -1;
            }
            ...
            {
                CYLOGE( "SPS size does not match NAL specified size %u",
                        i_data_size );
                return -1;
            }
            if( i_sps_pps_size + 4 + i_nal_size > i_out_buf_size )
            {
                CYLOGE( "Output SPS/PPS buffer too small" );
                return -1;
            }
