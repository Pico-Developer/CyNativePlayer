...
static AudioChannelMapEntry g_audio_channel_map[] = {
    { 4, CHANNEL_OUT_QUAD, "4-chan", "CHANNEL_OUT_QUAD" },
...
int audiotrack_get_native_output_sample_rate(JNIEnv *env)
{
    if (!env) {
        if (JNI_OK != SDL_JNI_SetupThreadEnv(&env)) {
            CYLOGE("%s: SetupThreadEnv failed", __func__);
            return -1;
        }
    }
...
SDL_Android_AudioTrack *SDL_Android_AudioTrack_new_from_spec(JNIEnv *env, SDL_Android_AudioTrack_Spec *spec)
{
    ...
    case CHANNEL_OUT_MONO:
        CYLOGI("SDL_Android_AudioTrack: %s", "CHANNEL_OUT_MONO");
        break;
    case CHANNEL_OUT_STEREO:
        CYLOGI("SDL_Android_AudioTrack: %s", "CHANNEL_OUT_STEREO");
        break;
    case CHANNEL_OUT_QUAD:
        CYLOGI("SDL_Android_AudioTrack: %s", "CHANNEL_OUT_QUAD");
        break;
    default:
        CYLOGE("%s: invalid channel %d", __func__, spec->channel_config);
        return NULL;
    }

    switch (spec->audio_format) {
    case ENCODING_PCM_16BIT:
        CYLOGI("SDL_Android_AudioTrack: %s", "ENCODING_PCM_16BIT");
        break;
    case ENCODING_PCM_8BIT:
        CYLOGI("SDL_Android_AudioTrack: %s", "ENCODING_PCM_8BIT");
        break;
#if 0
    case ENCODING_PCM_FLOAT:
        CYLOGI("SDL_Android_AudioTrack: %s", "ENCODING_PCM_FLOAT");
        if (sdk_int < IJK_API_21_LOLLIPOP) {
            CYLOGI("SDL_Android_AudioTrack: %s need API 21 or above", "ENCODING_PCM_FLOAT");
            return NULL;
        }
        break;
#endif
    default:
        CYLOGE("%s: invalid format %d", __func__, spec->audio_format);
        return NULL;
    }

    if (spec->sample_rate_in_hz <= 0) {
        CYLOGE("%s: invalid sample rate %d", __func__, spec->sample_rate_in_hz);
        return NULL;
    }

    SDL_Android_AudioTrack *atrack = (SDL_Android_AudioTrack*) mallocz(sizeof(SDL_Android_AudioTrack));
    if (!atrack) {
        CYLOGE("%s: mallocz faild.\n", __func__);
        return NULL;
    }
    atrack->spec = *spec;
    ...
    if (J4A_ExceptionCheck__catchAll(env) || min_buffer_size <= 0) {
        CYLOGE("%s: J4AC_AudioTrack__getMinBufferSize: return %d:", __func__, min_buffer_size);
        free(atrack);
        return NULL;
    }
    ...
    CYLOGI("%s: init volume as %f/(%f,%f)", __func__, init_volume, atrack->min_volume, atrack->max_volume);
...
bool SDL_Android_AudioTrack_getTimestamp(JNIEnv *env, SDL_Android_AudioTrack *atrack, int64_t *frame_position, int64_t *nano_timestamp) {
    jobject audio_timestamp = J4AC_AudioTimestamp__AudioTimestamp__catchAll(env);
    if (audio_timestamp == NULL)
        return false;
    jboolean ret_value = J4AC_AudioTrack__getTimestamp__catchAll(env, atrack->thiz, audio_timestamp);
    if (frame_position)
        *frame_position = J4AC_android_media_AudioTimestamp__framePosition__catchAll(env, audio_timestamp);
    if (nano_timestamp)
        *nano_timestamp = J4AC_android_media_AudioTimestamp__nanoTime__catchAll(env, audio_timestamp);
    J4A_DeleteLocalRef__p(env, &audio_timestamp);
    return ret_value;
}
