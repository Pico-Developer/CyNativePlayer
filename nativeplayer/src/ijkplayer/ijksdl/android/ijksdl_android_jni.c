...
static void SDL_JNI_ThreadDestroyed(void* value)
{
    ...
    if (env != NULL) {
        CYLOGE("%s: [%d] didn't call SDL_JNI_DetachThreadEnv() explicity\n", __func__, (int)gettid());
...
{
    // we couldn't have any cylog here, because cylog may use jni,
    // which is not available at this time

    // CYLOGI("%s\n", __func__);
    JavaVM *jvm = g_jvm;
    if (!jvm) {
        // CYLOGE("SDL_JNI_GetJvm: AttachCurrentThread: NULL jvm");
        return -1;
    }
    ...
    if ((*jvm)->AttachCurrentThread(jvm, &env, NULL) == JNI_OK) {
        // CYLOGI("attach env to thread[%d]", gettid());
        pthread_setspecific(g_thread_key, env);
        *p_env = env;
        return 0;
    }
    // CYLOGE("attach env to thread[%d] failed", gettid());
...
void SDL_JNI_DetachThreadEnv()
{
    ...
    if ((*jvm)->DetachCurrentThread(jvm) == JNI_OK) {
        // CYLOGI("detach env from thread[%d]", gettid());
    } else {
        // CYLOGE("detach env from thread[%d] failed", gettid());
    }
...
int SDL_JNI_ThrowException(JNIEnv* env, const char* className, const char* msg)
{
    ...
    if ((*env)->ExceptionCheck(env)) {
        ...
        if (exception != NULL) {
            CYLOGW("Discarding pending exception (%s) to throw", className);
    ...
    if (exceptionClass == NULL) {
        CYLOGE("Unable to find exception class %s", className);
        /* ClassNotFoundException now pending */
        goto fail;
    }

    if ((*env)->ThrowNew(env, exceptionClass, msg) != JNI_OK) {
        CYLOGE("Failed throwing '%s' '%s'", className, msg);
...
int SDL_Android_GetApiLevel()
{
    ...
    //     CYLOGE("SDL_Android_GetApiLevel: SetupThreadEnv failed");
    //     return 0;
    // }
