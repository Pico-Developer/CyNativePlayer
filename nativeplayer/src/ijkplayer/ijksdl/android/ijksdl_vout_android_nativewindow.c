...
static void func_free_l(SDL_Vout *vout)
{
    ...
    if (opaque) {
        ...
        if (opaque->native_window) {
            if (vout->connect_to_cpu) {
                int ret = ANativeWindow_disconnect_from_cpu(opaque->native_window);
                if (ret != 0) {
                    CYLOGE("%s: ANativeWindow_disconnect_from_cpu %p failed, ret: %d", __func__, opaque->native_window, ret);
                } else {
                    CYLOGI("%s: ANativeWindow_disconnect_from_cpu %p", __func__, opaque->native_window);
                }
            }
            // CYLOGI("%s: release nativewindow %p\n", __func__, opaque->native_window);
...
static int func_display_overlay_l(SDL_Vout *vout, SDL_VoutOverlay *overlay)
{
    ...
    if (!native_window) {
        if (!opaque->null_native_window_warned) {
            ...
            CYLOGW("func_display_overlay_l: NULL native_window");
        }
        return -1;
    } else {
        opaque->null_native_window_warned = 1;
    ...
    if (!overlay) {
        CYLOGE("func_display_overlay_l: NULL overlay");
        return -1;
    }

    if (overlay->w <= 0 || overlay->h <= 0) {
        CYLOGE("func_display_overlay_l: invalid overlay dimensions(%d, %d)", overlay->w, overlay->h);
        return -1;
    }
    ...
    IJK_EGL_terminate(opaque->egl);
    return SDL_Android_NativeWindow_display_l(vout, native_window, overlay);
}
...
static void SDL_VoutAndroid_SetNativeWindow_l(SDL_Vout *vout, ANativeWindow *native_window)
{
    ...
    // if (opaque->native_window){
    //     CYLOGI("%s: ANativeWindow_release %p\n", __func__, opaque->native_window);
    //     ANativeWindow_release(opaque->native_window);
    // }

    // if (native_window){
    //     CYLOGD("%s: ANativeWindow_acquire\n", __func__);
    //     ANativeWindow_acquire(native_window);
    // }

    ANativeWindow *prev_native_window = opaque->native_window;
    CYLOGI("%s: set nativewindow %p -> %p", __func__, prev_native_window, native_window);
    opaque->native_window = native_window;
    opaque->null_native_window_warned = 0;

    if (prev_native_window && vout->connect_to_cpu) {
        ANativeWindow_disconnect_from_cpu(prev_native_window);
        vout->connect_to_cpu = false;
    }
}
...
static int SDL_VoutAndroid_releaseBufferProxy_l(SDL_Vout *vout, SDL_AMediaCodecBufferProxy *proxy, bool render)
{
    ...
    if (!SDL_AMediaCodec_isSameSerial(opaque->acodec, proxy->acodec_serial)) {
        CYLOGW("%s: [%d] ???????? proxy %d: vout: %d idx: %d render: %s fake: %s",
            __func__,
            proxy->buffer_id,
            proxy->acodec_serial,
            SDL_AMediaCodec_getSerial(opaque->acodec),
    ...
    if (proxy->buffer_index < 0) {
        CYLOGE("%s: [%d] invalid AMediaCodec buffer index %d\n", __func__, proxy->buffer_id, proxy->buffer_index);
        return 0;
    ...
    if (proxy->buffer_info.size <= 0) {
        CYLOGI("%s: [%d] AMediaCodec buffer size %d, set render to false", __func__, proxy->buffer_id, proxy->buffer_info.size);
        render = false;
    }
