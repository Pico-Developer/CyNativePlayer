...
// PICO Spatial Audio
#include "OpenSLES_Spatial.h"

#ifdef SDLTRACE
#undef SDLTRACE
#define SDLTRACE(...)
//#define SDLTRACE CYLOGW
#endif

#define OPENSLES_BUFFERS 4 /* maximum number of buffers */
...
typedef struct SDL_Aout_Opaque {
    ...
    SLPlaybackRateItf               slPlabackRateItf;
    SLPlayItf                       slPlayItf;

    SLAndroidConfigurationItf       slAndroidConfigurationItf;
    SLAndroidSpatialAudioTrackItf   slAndroidSpatialAudioTrackItf;
...
static int aout_thread_n(SDL_Aout *aout)
{
    ...
    SLPlaybackRateItf              slPlabackRateItf = opaque->slPlabackRateItf;
    ...
    while (!opaque->abort_request) {
        ...
        if (slRet != SL_RESULT_SUCCESS) {
            CYLOGE("%s: slBufferQueueItf->GetState() failed\n", __func__);
        ...
        if (opaque->speed_changed) {
            ...
            SLpermille speed = lroundf(1000.f * opaque->speed);
            CYLOGI("slPlabackRateItf->SetPitch((%f) -> %d)\n", opaque->speed, (int)speed);
            slRet = (*slPlabackRateItf)->SetRate(slPlabackRateItf, speed);
            if (slRet!= SL_RESULT_SUCCESS) {
                CYLOGE("slPlabackRateItf->SetRate failed %d\n", (int)slRet);
                // just ignore error
            }
        }
        SDL_UnlockMutex(opaque->wakeup_mutex);
        ...
        if (opaque->need_flush) {
            ...
            if (slRet == SL_RESULT_SUCCESS) {
                ...
                CYLOGE("SL_RESULT_BUFFER_INSUFFICIENT\n");
            } else {
                CYLOGE("slBufferQueueItf->Enqueue() = %d\n", (int)slRet);
                break;
            }
        }
...
static void aout_close_audio(SDL_Aout *aout)
{
    ...
    if (opaque->slPlabackRateItf)
        opaque->slPlabackRateItf   = NULL;
...
static int aout_open_audio(SDL_Aout *aout, const SDL_AudioSpec *desired, SDL_AudioSpec *obtained)
{
    ...
    // CYLOGI("OpenSL-ES: native sample rate %d Hz\n", native_sample_rate);
    ...
    //     CYLOGW("OpenSL-ES: force resample %lu to native sample rate %d\n",
    ...
    case 1:
        ...
        break;
    default:
        CYLOGE("%s, invalid channel %d", __func__, desired->channels);
        goto fail;
    }
    ...
    SLInterfaceID spatialAudioTrackInterfaceId = NULL;
    if (&SL_IID_ANDROIDSPATIALAUDIOTRACK != NULL) {
        spatialAudioTrackInterfaceId = SL_IID_ANDROIDSPATIALAUDIOTRACK;
    }

    const SLInterfaceID ids2[] = { SL_IID_ANDROIDSIMPLEBUFFERQUEUE, SL_IID_VOLUME,
                                   SL_IID_PLAYBACKRATE, SL_IID_PLAY,
                                   SL_IID_ANDROIDCONFIGURATION,
                                   spatialAudioTrackInterfaceId};
    ...
    ret = (*slEngine)->CreateAudioPlayer(slEngine, &slPlayerObject, &audio_source,
                                &audio_sink, sizeof(ids2) / sizeof(*ids2) - (spatialAudioTrackInterfaceId == NULL ? 1 : 0),
                                ids2, req2);
    CHECK_OPENSL_ERROR(ret, "%s: slEngine->CreateAudioPlayer() failed", __func__);
    opaque->slPlayerObject = slPlayerObject;

    // PICO Spatial Audio
    ret = (*slPlayerObject)->GetInterface(slPlayerObject, SL_IID_ANDROIDCONFIGURATION, &opaque->slAndroidConfigurationItf);
    CYLOGI("%s: slAndroidConfigurationItf ret: %d", __func__, ret);
    if (spatialAudioTrackInterfaceId) {
        if(ret == SL_RESULT_SUCCESS) {
            switch (desired->spatial_audio_mode) {
                case 0:
                {
                    (*opaque->slAndroidConfigurationItf)->SetConfiguration(opaque->slAndroidConfigurationItf, SL_ANDROID_KEY_SPATIALIZATION_MODE,
                                                                        &SL_ANDROID_SPATIALIZATION_MODE_CHANNEL,
                                                                        sizeof(SLint32));
                } break;
                case 1:
                {
                    (*opaque->slAndroidConfigurationItf)->SetConfiguration(opaque->slAndroidConfigurationItf, SL_ANDROID_KEY_SPATIALIZATION_MODE,
                                                                        &SL_ANDROID_SPATIALIZATION_MODE_AMBIENT,
                                                                        sizeof(SLint32));
                } break;
                case 2:
                {
                    (*opaque->slAndroidConfigurationItf)->SetConfiguration(opaque->slAndroidConfigurationItf, SL_ANDROID_KEY_SPATIALIZATION_MODE,
                                                                        &SL_ANDROID_SPATIALIZATION_MODE_AMBIENT,
                                                                        sizeof(SLint32));
                    if(desired->channels == 4) {
                        (*opaque->slAndroidConfigurationItf)->SetConfiguration(opaque->slAndroidConfigurationItf, SL_ANDROID_KEY_AMBISONIC_CONTENT,
                                                                            &SL_ANDROID_AMBISNOIC_CONTENT_ENABLE,
                                                                            sizeof(SLint32));
                    }
                } break;
                case 3:
    ...
    // PICO Spatial Audio
    if (spatialAudioTrackInterfaceId) {
        ret = (*slPlayerObject)->GetInterface(slPlayerObject, SL_IID_ANDROIDSPATIALAUDIOTRACK, &opaque->slAndroidSpatialAudioTrackItf);
        CYLOGI("%s: slAndroidSpatialAudioTrackItf ret: %d", __func__, ret);
    }
    ...
    CHECK_OPENSL_ERROR(ret, "%s: slPlayerObject->GetInterface(SL_IID_VOLUME) failed", __func__);
     ...
     ret = (*slPlayerObject)->GetInterface(slPlayerObject, SL_IID_PLAYBACKRATE, &opaque->slPlabackRateItf);
     CHECK_OPENSL_ERROR(ret, "%s: slPlayerObject->GetInterface(SL_IID_PLAYBACKRATE) failed", __func__);
    ...
    CYLOGI("OpenSL-ES: bytes_per_frame  = %d bytes\n",  (int)opaque->bytes_per_frame);
    CYLOGI("OpenSL-ES: milli_per_buffer = %d ms\n",     (int)opaque->milli_per_buffer);
    CYLOGI("OpenSL-ES: frame_per_buffer = %d frames\n", (int)opaque->frames_per_buffer);
    CYLOGI("OpenSL-ES: bytes_per_buffer = %d bytes\n",  (int)opaque->bytes_per_buffer);
    CYLOGI("OpenSL-ES: buffer_capacity  = %d bytes\n",  (int)opaque->buffer_capacity);
...
static void aout_set_volume(SDL_Aout *aout, float left_volume, float right_volume)
{
    SDL_Aout_Opaque *opaque = aout->opaque;
    SDL_LockMutex(opaque->wakeup_mutex);
    CYLOGI("aout_set_volume(%f, %f)", left_volume, right_volume);
...
static double aout_get_latency_seconds(SDL_Aout *aout)
{
    ...
    if (!opaque->slBufferQueueItf) {
        CYLOGE("%s slBufferQueueItf is NULL", __func__);
    ...
    //CYLOGI("%s: state.count = %d\n", __func__, state.count);
    double latency = ((double)opaque->milli_per_buffer) * state.count / 1000;
    // the 125 ms is the kernel buffer and the hardware latency
    // but the latency is the true latency in application?
    return latency + 0.125;
}
...
// PICO Spatial Audio
static uint32_t func_set_spatial_audio_enable(SDL_Aout *aout, bool enable)
{
    int ret = -1;
    if (!aout) {
        return ret;
    ...
    SDL_Aout_Opaque *opaque = aout->opaque;
    if (!opaque->slAndroidSpatialAudioTrackItf) {
        return ret;
    }
    SDL_LockMutex(opaque->wakeup_mutex);
    CYLOGI("%s %d", __func__, enable);
    ret = (*opaque->slAndroidSpatialAudioTrackItf)->EnableSpatializer(opaque->slAndroidSpatialAudioTrackItf, enable);
    SDL_CondSignal(opaque->wakeup_cond);
    SDL_UnlockMutex(opaque->wakeup_mutex);
    return ret;
}

static uint32_t func_attach_to_entity(SDL_Aout *aout, int64_t entity_id)
{
    int ret = -1;
    if (!aout) {
        return ret;
    ...
    SDL_LockMutex(opaque->wakeup_mutex);
    CYLOGI("%s %lld", __func__, entity_id);
    ret = (*opaque->slAndroidSpatialAudioTrackItf)->AttachToEntity(opaque->slAndroidSpatialAudioTrackItf, entity_id);
    SDL_CondSignal(opaque->wakeup_cond);
    SDL_UnlockMutex(opaque->wakeup_mutex);
    return ret;
}

static uint32_t func_attach_to_container(SDL_Aout *aout, int64_t audio_bind_id)
{
    int ret = -1;
    if (!aout) {
        return ret;
    ...
    SDL_LockMutex(opaque->wakeup_mutex);
    CYLOGI("%s %lld", __func__, audio_bind_id);
    ret = (*opaque->slAndroidSpatialAudioTrackItf)->AttachToContainer(opaque->slAndroidSpatialAudioTrackItf, audio_bind_id);
    SDL_CondSignal(opaque->wakeup_cond);
    SDL_UnlockMutex(opaque->wakeup_mutex);
    return ret;
}
...
SDL_Aout *SDL_AoutAndroid_CreateForOpenSLES()
{
    ...
    aout->func_set_spatial_audio_enable = func_set_spatial_audio_enable;
    aout->func_attach_to_entity = func_attach_to_entity;
    aout->func_attach_to_container = func_attach_to_container;
