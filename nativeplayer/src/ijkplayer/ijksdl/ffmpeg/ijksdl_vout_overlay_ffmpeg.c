...
{
    CYLOGE("SDL_Overlay(ffmpeg): overlay_free_l(%p)\n", overlay);
    if (!overlay)
        return;
...
static int func_fill_frame(SDL_VoutOverlay *overlay, const AVFrame *frame)
{
    ...
    switch (overlay->format) {
        ...
        case SDL_FCC_I420:
            if (frame->format == AV_PIX_FMT_YUV420P || frame->format == AV_PIX_FMT_YUVJ420P) {
                // CYLOGE("direct draw frame");
                use_linked_frame = 1;
                dst_format = frame->format;
            } else {
                // CYLOGE("copy draw frame");
        ...
        case SDL_FCC_RV16:
            ...
            break;
        default:
            CYLOGE("SDL_VoutFFmpeg_ConvertPicture: unexpected overlay format %s(%d)",
    ...
    if (use_linked_frame) {
        ...
        if (!managed_frame) {
            CYLOGE("OOM in opaque_obtain_managed_frame_buffer");
            return -1;
        }
    ...
    // swscale / direct draw
    /*
     CYLOGE("ijk_image_convert w=%d, h=%d, df=%d, dd=%d, dl=%d, sf=%d, sd=%d, sl=%d",
     (int)frame->width,
     (int)frame->height,
     (int)dst_format,
    ...
    if (use_linked_frame) {
        ...
        if (opaque->img_convert_ctx == NULL) {
            CYLOGE("sws_getCachedContext failed");
            return -1;
        }
        ...
        if (!opaque->no_neon_warned) {
            ...
            CYLOGE("non-neon image convert %s -> %s", av_get_pix_fmt_name(frame->format), av_get_pix_fmt_name(dst_format));
        }
    }
...
SDL_VoutOverlay *SDL_VoutFFmpeg_CreateOverlay(int width, int height, int frame_format, SDL_Vout *display)
{
    ...
    if (!overlay) {
        CYLOGE("overlay allocation failed");
        return NULL;
    }
