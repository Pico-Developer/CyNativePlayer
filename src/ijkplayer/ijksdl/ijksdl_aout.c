...
double SDL_AoutGetTimestamp(SDL_Aout *aout)
{
    if (!aout)
        return 0;

    if (aout->func_get_time_stamp)
        return aout->func_get_time_stamp(aout);
...
// PICO Spatial Audio
int SDL_AoutSetSpatialAudioEnable(SDL_Aout *aout, bool enable)
{
    if (aout && aout->func_set_spatial_audio_enable) {
        return aout->func_set_spatial_audio_enable(aout, enable);
    }
    return SDL_AUDIO_MAX_CALLBACKS_PER_SEC;
}

int SDL_AoutAttachToEntity(SDL_Aout *aout, int64_t entityId)
{
    if (aout && aout->func_attach_to_entity) {
        return aout->func_attach_to_entity(aout, entityId);
    }
    return -1;
}

int SDL_AoutAttachToContainer(SDL_Aout *aout, int64_t containerId)
{
    if (aout && aout->func_attach_to_container) {
        return aout->func_attach_to_container(aout, containerId);
    }
    return -1;
}
