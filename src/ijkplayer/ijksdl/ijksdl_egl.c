...
static int IJK_EGL_getSurfaceWidth(IJK_EGL* egl)
{
    ...
    if (!eglQuerySurface(egl->display, egl->surface, EGL_WIDTH, &width)) {
        CYLOGE("[EGL] eglQuerySurface(EGL_WIDTH) returned error %d", eglGetError());
        return 0;
    }
...
static int IJK_EGL_getSurfaceHeight(IJK_EGL* egl)
{
    ...
    if (!eglQuerySurface(egl->display, egl->surface, EGL_HEIGHT, &height)) {
        CYLOGE("[EGL] eglQuerySurface(EGL_HEIGHT) returned error %d", eglGetError());
        return 0;
    }
...
static EGLBoolean IJK_EGL_setSurfaceSize(IJK_EGL* egl, int width, int height)
{
    ...
    if (width != egl->width || height != egl->height) {
        ...
        CYLOGI("ANativeWindow_setBuffersGeometry(w=%d,h=%d) -> (w=%d,h=%d);",
            egl->width, egl->height,
            width, height);
        int ret = ANativeWindow_setBuffersGeometry(egl->window, width, height, format);
        if (ret) {
            CYLOGE("[EGL] ANativeWindow_setBuffersGeometry() returned error %d", ret);
            return EGL_FALSE;
        }
...
static EGLBoolean IJK_EGL_makeCurrent(IJK_EGL* egl, EGLNativeWindowType window)
{
    if (window && window == egl->window &&
        ...
        if (!eglMakeCurrent(egl->display, egl->surface, egl->surface, egl->context)) {
            CYLOGE("[EGL] elgMakeCurrent() failed (cached)\n");
            return EGL_FALSE;
        }
    ...
    if (display == EGL_NO_DISPLAY) {
        CYLOGE("[EGL] eglGetDisplay failed\n");
        return EGL_FALSE;
    }
    ...
    if (!eglInitialize(display, &major, &minor)) {
        CYLOGE("[EGL] eglInitialize failed\n");
        return EGL_FALSE;   
    }
    CYLOGI("[EGL] eglInitialize %d.%d\n", (int)major, (int)minor);
    ...
    if (!eglChooseConfig(display, configAttribs, &config, 1, &numConfig)) {
        CYLOGE("[EGL] eglChooseConfig failed\n");
...
#ifdef __ANDROID__
    {
        ...
        if (!eglGetConfigAttrib(display, config, EGL_NATIVE_VISUAL_ID, &native_visual_id)) {
            CYLOGE("[EGL] eglGetConfigAttrib() returned error %d", eglGetError());
            eglTerminate(display);
            return EGL_FALSE;
        }
        ...
        CYLOGI("[EGL] ANativeWindow_setBuffersGeometry(f=%d);", native_visual_id);
        int ret = ANativeWindow_setBuffersGeometry(window, width, height, native_visual_id);
        if (ret) {
            CYLOGE("[EGL] ANativeWindow_setBuffersGeometry(format) returned error %d", ret);
            eglTerminate(display);
            return EGL_FALSE;
        }
    }
    ...
    if (surface == EGL_NO_SURFACE) {
        CYLOGE("[EGL] eglCreateWindowSurface failed\n");
        eglTerminate(display);
        return EGL_FALSE;
    }
    ...
    if (context == EGL_NO_CONTEXT) {
        CYLOGE("[EGL] eglCreateContext failed\n");
    ...
    if (!eglMakeCurrent(display, surface, surface, context)) {
        CYLOGE("[EGL] elgMakeCurrent() failed (new)\n");
    ...
    CYLOGI("[EGL] GLES2 extensions begin:\n");
    CYLOGI("[EGL]     GL_EXT_texture_rg: %d\n", egl->gles2_extensions[IJK_GLES2__GL_EXT_texture_rg]);
    CYLOGI("[EGL] GLES2 extensions end.\n");
#endif
...
static EGLBoolean IJK_EGL_prepareRenderer(IJK_EGL* egl, SDL_VoutOverlay *overlay)
{
    ...
    if (!IJK_GLES2_Renderer_isValid(opaque->renderer) ||
        ...
        if (!opaque->renderer) {
            CYLOGE("[EGL] Could not create render.");
            return EGL_FALSE;
        }

        if (!IJK_GLES2_Renderer_use(opaque->renderer)) {
            CYLOGE("[EGL] Could not use render.");
            IJK_GLES2_Renderer_freeP(&opaque->renderer);
            return EGL_FALSE;
        }
    }
    ...
    if (!IJK_EGL_setSurfaceSize(egl, overlay->w, overlay->h)) {
        CYLOGE("[EGL] IJK_EGL_setSurfaceSize(%d, %d) failed\n", overlay->w, overlay->h);
        return EGL_FALSE;
    }
...
static EGLBoolean IJK_EGL_display_internal(IJK_EGL* egl, EGLNativeWindowType window, SDL_VoutOverlay *overlay)
{
    ...
    if (!IJK_EGL_prepareRenderer(egl, overlay)) {
        CYLOGE("[EGL] IJK_EGL_prepareRenderer failed\n");
        return EGL_FALSE;
    }

    if (!IJK_GLES2_Renderer_renderOverlay(opaque->renderer, overlay)) {
        CYLOGE("[EGL] IJK_GLES2_render failed\n");
        return EGL_FALSE; 
    }
