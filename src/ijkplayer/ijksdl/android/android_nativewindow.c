...
static int android_render_yv12_on_yv12(ANativeWindow_Buffer *out_buffer, const SDL_VoutOverlay *overlay)
{
    ...
    // CYLOGE("stride:%d/%d, size:%d/%d", dst_y_stride, dst_c_stride, dst_y_size, dst_c_size);
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        if (dst_line_size == src_line_size) {
            ...
            // CYLOGE("sdl_image_copy_plane %p %p %d", dst_pixels, src_pixels, dst_plane_size);
            memcpy(dst_pixels, src_pixels, plane_size);
        } else {
            // TODO: 9 padding
            int bytewidth = IJKMIN(dst_line_size, src_line_size);

            // CYLOGE("av_image_copy_plane %p %d %p %d %d %d", dst_pixels, dst_line_size, src_pixels, src_line_size, bytewidth, line_height);
...
static int android_render_rgb_on_rgb(ANativeWindow_Buffer *out_buffer, const SDL_VoutOverlay *overlay, int bpp)
{
    ...
    if (dst_line_size == src_line_size) {
        ...
        // CYLOGE("android_render_rgb_on_rgb (pix-match) %p %p %d", dst_pixels, src_pixels, plane_size);
        memcpy(dst_pixels, src_pixels, plane_size);
    } else {
        // TODO: 9 padding
        int bytewidth = IJKMIN(dst_line_size, src_line_size);

        // CYLOGE("android_render_rgb_on_rgb (pix-mismatch) %p %d %p %d %d %d", dst_pixels, dst_line_size, src_pixels, src_line_size, bytewidth, min_height);
...
int SDL_Android_NativeWindow_display_l(SDL_Vout *vout, ANativeWindow *native_window, SDL_VoutOverlay *overlay)
{
    int retval;
    ...
    if (!overlay) {
        CYLOGE("SDL_Android_NativeWindow_display_l: NULL overlay");
        return -1;
    }

    if (overlay->w <= 0 || overlay->h <= 0) {
        CYLOGE("SDL_Android_NativeWindow_display_l: invalid overlay dimensions(%d, %d)", overlay->w, overlay->h);
        return -1;
    }
    ...
    if (!overlayDesc) {
        CYLOGE("SDL_Android_NativeWindow_display_l: unknown overlay format: %d", overlay->format);
        return -1;
    }
    ...
    if (!voutDesc || voutDesc->hal_format != overlayDesc->hal_format) {
        CYLOGD("ANativeWindow_setBuffersGeometry: w=%d, h=%d, f=%.4s(0x%x) => w=%d, h=%d, f=%.4s(0x%x)",
        ...
        if (retval < 0) {
            CYLOGE("SDL_Android_NativeWindow_display_l: ANativeWindow_setBuffersGeometry: failed %d", retval);
            return retval;
        }

        if (!voutDesc) {
            CYLOGE("SDL_Android_NativeWindow_display_l: unknown hal format %d", curr_format);
            return -1;
        }
    }
    ...
    if (retval < 0) {
        CYLOGE("SDL_Android_NativeWindow_display_l: ANativeWindow_lock: failed %d", retval);
        return retval;
    }
    vout->connect_to_cpu = true;


    if (out_buffer.width != buff_w || out_buffer.height != buff_h) {
        CYLOGE("unexpected native window buffer (%p)(w:%d, h:%d, fmt:'%.4s'0x%x), expecting (w:%d, h:%d, fmt:'%.4s'0x%x)",
            native_window,
    ...
    if (retval < 0) {
        CYLOGE("SDL_Android_NativeWindow_display_l: ANativeWindow_unlockAndPost: failed %d", retval);
        return retval;
    }
...
int32_t ANativeWindow_disconnect_from_cpu(ANativeWindow *w) {
    typedef int32_t (*PFN_perform)(struct ANativeWindow*, int, ...);
    // 152 is the offset of the field `perform` in ANativeWindow structure,
    // 152 == offsetof(struct ANativeWindow, perform)
    // pls refer to https://cs.android.com/android/platform/superproject/main/+/main:frameworks/native/libs/nativewindow/include/system/window.h;l=471
    PFN_perform perform = *(PFN_perform*)((void *)w + 152);
    return perform(w, 14 /* NATIVE_WINDOW_DISCONNECT */, 2 /* NATIVE_WINDOW_API_CPU */);
}
