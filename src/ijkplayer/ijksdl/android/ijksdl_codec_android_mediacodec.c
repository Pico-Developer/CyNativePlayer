    ...
    uint32_t flags)
{
    ...
    sdl_amedia_status_t ret = acodec->func_configure_surface(acodec, aformat, android_surface, crypto, flags);
    acodec->is_configured = true;
    acodec->is_started    = false;
    return ret;
}

sdl_amedia_status_t SDL_AMediaCodec_set_surface(SDL_AMediaCodec* acodec, void *android_surface) {
    assert(acodec->func_set_surface);
    sdl_amedia_status_t ret = acodec->func_set_surface(acodec, android_surface);
    return ret;
}
