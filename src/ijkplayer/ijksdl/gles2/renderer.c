...
static void IJK_GLES2_printProgramInfo(GLuint program)
{
    ...
    if (!info_len) {
        CYLOGE("[GLES2][Program] empty info\n");
        return;
    }
...
#if 0
    if (renderer->vertex_shader)    CYLOGW("[GLES2] renderer: vertex_shader not deleted.\n");
    if (renderer->fragment_shader)  CYLOGW("[GLES2] renderer: fragment_shader not deleted.\n");
    if (renderer->program)          CYLOGW("[GLES2] renderer: program not deleted.\n");

    for (int i = 0; i < IJK_GLES2_MAX_PLANE; ++i) {
        if (renderer->plane_textures[i])
            CYLOGW("[GLES2] renderer: plane texture[%d] not deleted.\n", i);
    }
#endif
...
IJK_GLES2_Renderer *IJK_GLES2_Renderer_create(SDL_VoutOverlay *overlay)
{
    ...
    switch (overlay->format) {
        ...
        default:
            CYLOGE("[GLES2] unknown format %4s(%d)\n", (char *)&overlay->format, overlay->format);
            return NULL;
    }
...
static void IJK_GLES2_Renderer_Vertices_apply(IJK_GLES2_Renderer *renderer)
{
    switch (renderer->gravity) {
        ...
        case IJK_GLES2_GRAVITY_RESIZE:
            ...
            return;
        default:
            CYLOGE("[GLES2] unknown gravity %d\n", renderer->gravity);
            IJK_GLES2_Renderer_Vertices_reset(renderer);
            return;
    }
    ...
    {
        CYLOGE("[GLES2] invalid width/height for gravity aspect\n");
        IJK_GLES2_Renderer_Vertices_reset(renderer);
        return;
    }
...
{
    CYLOGE("IJK_GLES2_Renderer_TexCoords_cropRight\n");
    renderer->texcoords[0] = 0.0f;
    renderer->texcoords[1] = 1.0f;
