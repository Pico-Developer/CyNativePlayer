...
retry:
        if (frame_queue_nb_remaining(&is->pictq) == 0) {
            ...
            if (is->seek_info.need_quickly_skip_frame) {
            ...
            if (vp->serial != is->videoq.serial || is->seek_info.curr_seek_mode == FF_SEEK_MODE_ACCURATE && vp->pts < is->seek_info.seek_pos) {
...
reload:
    ...
    } while (af->serial != is->audioq.serial || is->seek_info.curr_seek_mode == FF_SEEK_MODE_ACCURATE && af->pts < is->seek_info.seek_pos);
...
static void sdl_audio_callback(void *opaque, Uint8 *stream, int len)
{
    ...
    if (ffp->stereo_volume_changed && (av_gettime_relative() - is->seek_info.audio_flush_timestamp > 200000 ||
...
static void decide_seek_mode_policy(FFPlayer *ffp) {
    ...
    FFSeekMode last_mode = is->seek_info.curr_seek_mode;
...
int ffp_seek_to_l(FFPlayer *ffp, long msec)
{
    ...
    ffp->last_get_position = 0;
    is->seek_info.seek_pos = msec / 1000.0;
...
long ffp_get_current_position_l(FFPlayer *ffp)
{
    ...
    adjust_pos = FFMIN((long)adjust_pos, ffp_get_duration_l(ffp));
...
void ffp_set_seek_mode(FFPlayer *ffp, FFSeekMode seek_mode)
{
    ...
    if (ffp->is && seek_mode != FF_SEEK_MODE_AUTO) ffp->is->seek_info.curr_seek_mode = seek_mode;
}
